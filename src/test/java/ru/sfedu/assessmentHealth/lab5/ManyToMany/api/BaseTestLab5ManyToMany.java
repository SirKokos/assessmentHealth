...
import ru.sfedu.assessmentHealth.lab5.ManyToMany.api.HibernateDataProviderLab5ManyToMany;
...
@Slf4j
public class BaseTestLab5ManyToMany {
    HibernateDataProviderLab5ManyToMany hibernateDataProviderLab5ManyToMany = new HibernateDataProviderLab5ManyToMany();
    static {
        PropertyConfig.setConfigPath(Const.NAME_PROPERTY_FILE);
        HibernateUtil.setPathConfig(PropertyConfig.getPropertyValue(Const.LAB5_MANY_TO_MANY,Const.NAME_PROPERTY_FILE));
    }
...
//        schedule.setId(Integer.valueOf(Const.TEST_SCHEDULE_DATA_ID));
    ...
    protected List<Preparation> generateEntity(){
        ArrayList<Preparation> preparationArrayList = new ArrayList<>();
        try {
            preparationArrayList = (ArrayList<Preparation>) Stream.generate( ()-> getPreparation()  )
                    .limit(1000)
                    .collect(Collectors.toList());
        }catch (Exception e){
            log.error("generateEntity [2]: error {}",preparationArrayList);
        }
        return preparationArrayList;
    }
